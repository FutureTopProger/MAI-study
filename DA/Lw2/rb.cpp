...
TRBtree::Insert(TNode& newNode) {
    TNode* preNode = this->nil; //this просто чтобы читаемо было
...
TRBtree::Serialize(const char* buffer) {
    std::ofstream output(buffer); //вызывается конструктор потока аутпут и он считывает инфу из файла
    if(output.is_open()) {
...
// DELETE
// void
// TRBtree::Postorder(TNode* p, int indent) {
//     if(p != NULL) {
//         if(p->left) {
